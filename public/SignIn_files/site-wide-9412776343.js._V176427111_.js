/* begin mergedCore-1.2.6/core.js */
...
if(all_loaded){}};
}this.addLogical=function(functionality,urls){if(urls){;
}else{;
...
i<urlList.length;
i++){if(urlList[i]){plUrls.push(urlList[i]);
...
if(window.ue&&bootstrapAmznJQ&&bootstrapAmznJQ.ueJQMetrics&&window.ues&&window.uex){ues("wb","jQueryActive",1);
uex("ld","jQueryActive");
...
var mouseTracker=function(){var regions=[];
var cursor={x:0,y:0};
var scroll=[0,0];
var listening=false;
...
i<regions.length;
i++){var r=regions[i];
var inside=$.grep(r.rects,function(n){return cursor.x>=n[0]&&cursor.y>=n[1]&&cursor.x<n[0]+n[2]&&cursor.y<n[1]+n[3];
}).length>0;
if(r.inside!==null&&inside&&!r.inside&&r.mouseEnter){r.inside=r.mouseEnter();
}else{if(r.inside!==null&&!inside&&r.inside&&r.mouseLeave){r.inside=!r.mouseLeave(immediately);
}}}};
var startListening=function(){scroll=[$(window).scrollLeft(),$(window).scrollTop()];
$(document).mousemove(function(e){cursor={x:e.pageX,y:e.pageY};
check();
});
$(document).scroll(function(e){cursor.x+=($(window).scrollLeft()-scroll[0]);
cursor.y+=($(window).scrollTop()-scroll[1]);
...
amznJQ.available("jQuery",function(){(function(window,undefined){var $=window.jQuery,merchRE=/^me=/,refre=/(ref=[-\w]+)/,ddre=/_dd_/,ddaliasre=/(dd_[a-z]{3,4})(_|$)[\w]*/,deptre=/\{department\}/g,slashre=/\+/g,aliasre=/search-alias\s*=\s*([\w-]+)/,dcs="#ddCrtSel",sdpc="searchDropdown_pop_conn",tostr=Object.prototype.toString,ddBox;
...
node=$("<div id='srch_sggst'/>").css({"background-color":"#FFF",border:"1px solid #5C8295",color:"#000",display:"none",left:p.left,position:"absolute","text-align":"left",top:p.top+d.height,width:d.width,"z-index":250}).appendTo(sb.parent());
$(window).resize(move);
...
var SB=function(sb,h){var curText,hasFocus,curSel;
init();
function init(){sb.keydown(keyDown).keyup(keyUp).keypress(keyPress).blur(blurHandler).focus(focusHandler);
curText=kw();
...
if(val!=curText&&(dontCheckCurSel||val!=curSel)){h.change(curText=val);
}}function focusHandler(e){hasFocus=true;
}function blurHandler(e){h.dismiss();
hasFocus=false;
}this.keyword=function(k){return kw(k);
};
this.size=function(){return{width:sb.outerWidth(),height:sb.outerHeight()};
};
this.pos=function(){return sb.position();
};
...
lookup(opt,"sb");
if(!opt.sb){return;
}searchBox=new SB(opt.sb,{adjust:move,hide:hideSuggestions,dismiss:dismissSuggestions,change:(staticContent?update:delayUpdate),submitEnabled:submitEnabled,keyPress:keyPress});
...
}catch(e){}}function hideSuggestions(){!opt.ime&&hideSuggestionsDiv();
...
if(!kw.length){hideSuggestionsDiv();
hideNoMatches();
}else{first=-1;
...
timer=null;
}timer=setTimeout(function(){return(function(){searchJSONSuggest();
timer=null;
crtSel=-1;
});
}(),100);
}function submitEnabled(){var s=opt.submit;
...
}function findBin(){var low=0,high=names.length-1,mid=-1,dataPrefix="",crtPrefix=keyword().toLowerCase(),len=crtPrefix.length;
while(low<=high){mid=Math.floor((low+high)/2);
dataPrefix=names[mid].substr(0,len).toLowerCase();
...
++i;
}while((n=names[i])&&!n.toLowerCase().indexOf(crtPrefix));
}}function findSeq(){var crtPrefix=keyword().toLowerCase(),rexp=new RegExp("(^|(?:\\s))"+crtPrefix,"i"),i=0,len=names.length,n;
for(;
i<len;
i++){n=names[i];
if(n.match(rexp)){suggestionList.push({keyword:n,i:i});
if(first==-1){first=i;
}}}}function checkForExactMatch(){var state="disabled";
if(curSize){var sg=suggestionList[0].keyword,kw=keyword();
...
searchSuggest.show();
if(!staticContent){h+='<div id="sugdivhdr" align="right"> '+opt.sugText+"</div>";
}}else{showNoMatches();
}for(i=0;
i<curSize;
...
}if(curSize>0){h+='<div id="sugdivhdr" align="right">&nbsp;</div>';
...
if(indexInList>=0){var issMode,kw=keyword();
...
}}function suggestOver(event){this.style.cursor="default";
...
}function highlightSuggestion(suggestion){addRemoveClass(suggestion,"suggest_link_over","suggest_link");
}function unhighlightSuggestion(suggestion){addRemoveClass(suggestion,"suggest_link","suggest_link_over");
}function highlightCurrentSuggestion(updateSearchBox){updateSearchBox&&updateCrtSuggestion();
...
}function getPrefixPos(str,substr){return str.toLowerCase().indexOf(substr.toLowerCase());
...
}function hideSuggestionsDiv(){if(suggestRequest){suggestRequest.cleanup();
suggestRequest=null;
}curSize=0;
searchSuggest.hide();
crtSel=-1;
...
input(opt.form,"frmDynamic",param.name,unescape(decodeURI(param.value)),1);
...
if(!supportedSearchAlias(alias)){return;
}if(!kw||!kw.length){hideSuggestionsDiv();
return;
...
}return{suggest:bindSuggest,keypress:bindKeypress,submit:bindSubmit,blur:blur,keyword:keyword,searchAlias:searchAlias,stop:stop,start:start,encoding:encoding,initStaticSuggestions:initStatic,initDynamicSuggestions:initDynamic,updateAutoCompletion:updateCompletion,init:init};
};
...
}function addRemoveClass(n,a,r){n.removeClass(r).addClass(a);
